...
def _safe_get(sequence: Sequence[Any] | None, index: int, default: Any = None) -> Any:
    if sequence is None:
        return default
    try:
...
def _descriptor_for_atom(descriptor: Any, atom_index: int, carbene_index: Optional[int] = None) -> Any:
    ...
    if isinstance(descriptor, (list, tuple)) and descriptor and isinstance(descriptor[0], (list, tuple, np.ndarray)):
        return _safe_get(descriptor, atom_index)

    if carbene_index is not None and atom_index == carbene_index:
        return descriptor
...
def _identify_carbene_center(mol: Optional[Chem.Mol]) -> Optional[Dict[str, Any]]:
    if mol is None:
        return None
    ...
    for atom in mol.GetAtoms():
        ...
        return {
            "carbene": atom.GetIdx(),
            "heteroatoms": [neighbor.GetIdx() for neighbor in hetero_neighbors],
        }
...
def _default_node_record(atom_index: int, qm_data: "DictData") -> Dict[str, Any]:
    record = {
        ...
        "atomic_number": _safe_get(qm_data.atomic_numbers, atom_index),
        "atom_label": _safe_get(qm_data.atom_labels, atom_index),
        "position": _safe_get(qm_data.xyz_coordinates, atom_index),
        "atomic_mass": _safe_get(qm_data.atomic_masses, atom_index),
        "electronegativity": _safe_get(qm_data.electronegativities, atom_index),
        "covalent_radius": _safe_get(qm_data.covalent_radii, atom_index),
    }
    ...
    record.update({
        ...
        "natural_charge": _safe_get(getattr(qm_data, "natural_charges", None), atom_index),
        "core_population": _safe_get(getattr(qm_data, "core_populations", None), atom_index),
        "valence_population": _safe_get(getattr(qm_data, "valence_populations", None), atom_index),
        "rydberg_population": _safe_get(getattr(qm_data, "rydberg_populations", None), atom_index),
        "total_population": _safe_get(getattr(qm_data, "total_populations", None), atom_index),
        "core_orbital_occupancy": _safe_get(getattr(qm_data, "core_orbital_occupancies", None), atom_index),
        "core_orbital_energy": _safe_get(getattr(qm_data, "core_orbital_energies", None), atom_index),
    })

    lone_pair_occupancies = getattr(qm_data, "lone_pair_occupancies", None)
    lone_pair_energies = getattr(qm_data, "lone_pair_energies", None)
    lone_pair_occ = _safe_get(lone_pair_occupancies, atom_index)
    lone_pair_en = _safe_get(lone_pair_energies, atom_index)
    record.update({
        "lone_pair_1_occupancy": _safe_get(lone_pair_occ, 0) if isinstance(lone_pair_occ, (list, tuple)) else None,
        "lone_pair_1_energy": _safe_get(lone_pair_en, 0) if isinstance(lone_pair_en, (list, tuple)) else None,
        "lone_pair_2_occupancy": _safe_get(lone_pair_occ, 1) if isinstance(lone_pair_occ, (list, tuple)) else None,
        "lone_pair_2_energy": _safe_get(lone_pair_en, 1) if isinstance(lone_pair_en, (list, tuple)) else None,
    })

    nmr = qm_data.NMR_shieldings or {}
    record.update({
        "proton_isotropic": _safe_get(nmr.get("proton_isotropic"), atom_index),
        "proton_anisotropic": _safe_get(nmr.get("proton_anisotropic"), atom_index),
        "carbon_isotropic": _safe_get(nmr.get("carbon_isotropic"), atom_index),
        "carbon_anisotropic": _safe_get(nmr.get("carbon_anisotropic"), atom_index),
    })

    fukui = qm_data.Fukui_indices or {}
    record.update({
        "fukui_f_plus": _safe_get(fukui.get("f_plus"), atom_index),
        "fukui_f_minus": _safe_get(fukui.get("f_minus"), atom_index),
        "fukui_f_zero": _safe_get(fukui.get("f_zero"), atom_index),
    })
...
class DictData:
    ...
    def __init__(self, qm_data_dict: Optional[Dict[str, Any]] = None):
        """
...
class GraphInfo(GraphBase):
    def __init__(self, qm_data: Optional[DictData] = None):
        ...
        natural_population_totals = getattr(qm_data, 'natural_population_totals', None)
        if natural_population_totals and isinstance(natural_population_totals, dict) and 'total_charge' in natural_population_totals:
            self.total_charge = natural_population_totals['total_charge']
...
class Edge(GraphBase):
    def __init__(self, qm_data: Optional[DictData] = None):
        ...
        super().__init__(qm_data)
        if qm_data is None:
            self.bond_distance_matrix = None
            self.smiles = None
            self.num_nodes = None
            return
    ...
    def get_edge_features(self, bond_order_threshold: float = 0.4, max_distance: float = 3.0) -> list[dict]:
        ...
        if not edge_data or not edge_data['atom_i_list']:
            if self.qm_data is not None and self.bond_distance_matrix is not None and self.qm_data.wiberg_bond_order_matrix is not None:
                atom_i_indices, atom_j_indices, distances, bond_orders = analyze_bond_orders(
                    self.bond_distance_matrix,
                    self.qm_data.wiberg_bond_order_matrix,
                    bond_order_threshold,
                    max_distance,
                )
                edge_list = []
                conventional_bond_orders = get_conventional_bond_orders(bond_orders) if bond_orders else []
                num_2c_bds_list = []
                for i, j in zip(atom_i_indices, atom_j_indices):
                    num_2c_bds_list.append(self.qm_data.number_of_2C_BDs_matrix[i][j] if self.qm_data.number_of_2C_BDs_matrix is not None else None)

                for idx, (i, j, distance_val) in enumerate(zip(atom_i_indices, atom_j_indices, distances)):
                    edge_list.append({
                        "atom_i": i,
                        "atom_j": j,
                        "distance": distance_val,
                        "edge_type": "catalyst",
                        "bond_order": bond_orders[idx] if idx < len(bond_orders) else None,
                        "conventional_bond_order": conventional_bond_orders[idx] if idx < len(conventional_bond_orders) else None,
                        "num_2C_BDs": num_2c_bds_list[idx] if idx < len(num_2c_bds_list) else None,
                        "bonding_orbital_occupancy": None,
                        "bonding_orbital_energy": None,
                        "antibonding_orbital_occupancy": None,
                        "antibonding_orbital_energy": None,
                    })
                return edge_list
            return []
    ...
    def get_edge_features_ML(self, bond_order_threshold: float = 0.4, max_distance: float = 3.0) -> list[dict]:
        ...
        edge_data = self.qm_data.get_precomputed_edge_features(bond_order_threshold, max_distance)
        if not edge_data or not edge_data['atom_i_list']:
            return self.get_edge_features(bond_order_threshold, max_distance)
...
class Targets(GraphBase):
    ...
    def __init__(self, qm_data: Optional[DictData] = None):
        ...
        self.morfeus = qm_data.morfeus
        self.IR_stats = qm_data.IR_stats
        self.NMR_shieldings = qm_data.NMR_shieldings
        self.dipole_moment = qm_data.dipole_moment
    ...
    def _flatten_catalyst_targets(self) -> Dict[str, Any]:
        homo_lumo = self.homo_lumo or {}
        morfeus = self.morfeus or {}
        ir_stats = self.IR_stats or {}
        dipole_moment = self.dipole_moment or {}
        ie_ea = self.IE_EA or {}
        polarizability = self.polarizability or {}

        flattened = {
            "HOMO": homo_lumo.get("HOMO"),
            "LUMO": homo_lumo.get("LUMO"),
            "HOMO_LUMO_gap": homo_lumo.get("HOMO_LUMO_gap"),
            "buried_volume": morfeus.get("buried_volume"),
            "fraction_vbur": morfeus.get("fraction_vbur"),
            "free_volume": morfeus.get("free_volume"),
            "sasa_area": morfeus.get("sasa_area"),
            "sasa_volume": morfeus.get("sasa_volume"),
            "IR_freq_mean": _nested_get(ir_stats, "frequencies", "mean"),
            "IR_freq_max": _nested_get(ir_stats, "frequencies", "max"),
            "IR_freq_min": _nested_get(ir_stats, "frequencies", "min"),
            "IR_freq_std": _nested_get(ir_stats, "frequencies", "std"),
            "IR_freq_median": _nested_get(ir_stats, "frequencies", "median"),
            "IR_eps_mean": _nested_get(ir_stats, "eps", "mean"),
            "IR_eps_max": _nested_get(ir_stats, "eps", "max"),
            "IR_eps_min": _nested_get(ir_stats, "eps", "min"),
            "IR_eps_std": _nested_get(ir_stats, "eps", "std"),
            "IR_eps_median": _nested_get(ir_stats, "eps", "median"),
            "IR_intensity_mean": _nested_get(ir_stats, "intensity", "mean"),
            "IR_intensity_max": _nested_get(ir_stats, "intensity", "max"),
            "IR_intensity_min": _nested_get(ir_stats, "intensity", "min"),
            "IR_intensity_std": _nested_get(ir_stats, "intensity", "std"),
            "IR_intensity_median": _nested_get(ir_stats, "intensity", "median"),
            "IR_t2_mean": _nested_get(ir_stats, "t2", "mean"),
            "IR_t2_max": _nested_get(ir_stats, "t2", "max"),
            "IR_t2_min": _nested_get(ir_stats, "t2", "min"),
            "IR_t2_std": _nested_get(ir_stats, "t2", "std"),
            "IR_t2_median": _nested_get(ir_stats, "t2", "median"),
            "dipole_x": dipole_moment.get("dipole_x"),
            "dipole_y": dipole_moment.get("dipole_y"),
            "dipole_z": dipole_moment.get("dipole_z"),
            "dipole_magnitude": dipole_moment.get("dipole_magnitude"),
            "isotropic_polarizability": polarizability.get("isotropic_polarizability"),
            "ionization_energy": ie_ea.get("ionization_energy"),
            "electron_affinity": ie_ea.get("electron_affinity"),
            "hardness": ie_ea.get("hardness"),
            "chemical_potential": ie_ea.get("chemical_potential"),
            "electronegativity": ie_ea.get("electronegativity"),
            "electrophilicity": ie_ea.get("electrophilicity"),
        }
    ...
    def get_targets(self) -> dict:
        ...
        targets_dict.update({
            "natural_minimal_basis": self.natural_minimal_basis,
            "natural_rydberg_basis": self.natural_rydberg_basis,
            "total_core_population": self.total_core_population,
            "total_valence_population": self.total_valence_population,
            "total_rydberg_population": self.total_rydberg_population,
