...
PATTERNS = {
    ...
    "orbital_start": re.compile(r"^\s*ORBITAL ENERGIES\s*$"),
    "orbital_row": re.compile(
        r"^\s*\d+\s+(?P<occ>\d+\.\d+)\s+(?P<eh>-?\d+\.\d+)\s+(?P<ev>-?\d+\.\d+)\s*$"
),
    "ir_start": re.compile(r"^\s*IR SPECTRUM\s*$"),
    "ir_end": re.compile(r"^\s*\* The epsilon \(eps\) is given for a Dirac delta lineshape\.\s*$"),
    "ir_row": re.compile(
        r"^\s*\d+:\s+(?P<freq>-?\d+\.\d+)\s+"
        r"(?P<eps>-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s+"
        r"(?P<int>-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s+"
        r"(?P<t2>-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s+\(.*\)\s*$"
    ),
    "nmr_start": re.compile(r"^\s*CHEMICAL SHIELDING SUMMARY \(ppm\)\s*$"),
    "nmr_end": re.compile(r"^\s*NMR shielding tensor and spin rotation calculation done\b"),
    "nmr_row": re.compile(
        r"^\s*(?P<idx>\d+)\s+(?P<el>[A-Z][a-z]?)\s+"
        r"(?P<iso>-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s+"
        r"(?P<aniso>-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*$"
    ),
    "dipole_start": re.compile(r"^\s*DIPOLE MOMENT\s*$"),
    "dipole_end": re.compile(r"^\s*Rotational spectrum\b"),
    "dipole_total_row": re.compile(
        r"^\s*Total Dipole Moment\s*:\s*"
        r"(?P<x>-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s+"
        r"(?P<y>-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s+"
        r"(?P<z>-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*$"
    ),
    "dipole_mag": re.compile(
        r"^\s*Magnitude \(Debye\)\s*:\s*(?P<mag>-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*$"
    ),
    "polar_start": re.compile(r"^\s*STATIC POLARIZABILITY TENSOR \(Dipole/Dipole\)\s*$"),
    "polar_value": re.compile(
        r"^\s*Isotropic polarizability\s*:\s*(?P<value>-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*$"
    ),
    "final_sp_energy": re.compile(
        r"^\s*FINAL SINGLE POINT ENERGY\s+(?P<energy>-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*$"
    ),
}
...
# ------------------------- featuresyrup additional functions -----------------------------------------

def identify_carbene_center(mol):
    """
    Helper function. 
    Identify the carbene center in an NHC. 
    """
    for atom in mol.GetAtoms():
        ...
        if len(hetero_neighbors) != 2:
            continue
        if not atom.IsInRing():
            continue
        if not all(neighbor.IsInRing() for neighbor in hetero_neighbors):
            continue

        return {
            "carbene": atom.GetIdx(),
            "heteroatoms": [n.GetIdx() for n in hetero_neighbors],
        }

    return None

def parse_orbital_energies(output_path: str) -> dict[str, float]:
    """
    Find the last ORBITAL ENERGIES block in an ORCA .out file and return:
      - HOMO energy in eV as a string
      - LUMO energy in eV as a string
      - HOMO-LUMO gap in eV as a float
    """
    text = Path(output_path).read_text(errors="ignore").splitlines()

    start_idx = None
    for i, line in enumerate(text):
        if PATTERNS["orbital_start"].match(line):
            start_idx = i

    if start_idx is None:
        raise ValueError("Could not find 'ORBITAL ENERGIES' in the output file.")

    homo_ev = None
    lumo_ev = None

    for line in text[start_idx + 1 :]:
        m = PATTERNS["orbital_row"].match(line)
        if not m:
            continue
        ...
        if occ == "2.0000":
            homo_ev = ev
        elif occ == "0.0000" and lumo_ev is None:
            lumo_ev = ev
            break

    if homo_ev is None:
        raise ValueError("Could not find the last OCC=2.0000 orbital.")
    if lumo_ev is None:
        raise ValueError("Could not find the first OCC=0.0000 orbital.")

    gap_ev = float(lumo_ev) - float(homo_ev)
    return {"HOMO": homo_ev, "LUMO": lumo_ev, "HOMO_LUMO_gap": gap_ev}


def calculate_morfeus_descriptors(xyz_file: str, smiles: str) -> dict[str, float]:
    """
    Calculate a combined set of Morfeus descriptors for a molecule.
    ...
    Notes:
      - Uses a carbene center if one can be identified.
      - Morfeus uses 1-indexed atom indices.
      - RDKit uses 0-indexed atom indices.
    """
    if morfeus is None:
        raise ImportError("morfeus is required for calculate_morfeus_descriptors")

    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        raise ValueError(f"Could not parse SMILES: {smiles}")

    carbene_info = identify_carbene_center(mol)
    if carbene_info is None:
        raise ValueError("Could not identify a carbene center in the molecule.")

    # RDKit is 0-indexed, Morfeus is 1-indexed
    carbene_index_rdkit = carbene_info["carbene"]
    carbene_index = carbene_index_rdkit + 1
    heteroatom_indices_rdkit = carbene_info["heteroatoms"]
    heteroatom_indices = [idx + 1 for idx in heteroatom_indices_rdkit]

    # Read xyz data
    atom_list, atomic_numbers, xyz_coordinates, num_atoms, atom_labels = read_xyz_file(xyz_file)

    # --- Buried volume ---
    z_axis_atoms = heteroatom_indices if heteroatom_indices else []
    xz_plane_atoms = [heteroatom_indices[0]] if heteroatom_indices else []

    bv = morfeus.BuriedVolume(
        atomic_numbers,
        xyz_coordinates,
        metal_index=carbene_index,
        include_hs=False,
        radius=3.5,
    ...
    return {
        "buried_volume": float(bv.buried_volume),
        "fraction_vbur": float(bv.fraction_buried_volume),
        "free_volume": float(bv.free_volume),
        "sasa_area": float(sasa.area),
...
def get_IR_frequencies(output_file: str) -> dict:
    ...
    Returns:
      ...
      - stats
    """
    lines = Path(output_file).read_text(errors="ignore").splitlines()

    start_idx = None
    for i, line in enumerate(lines):
        if PATTERNS["ir_start"].match(line):
            start_idx = i

    if start_idx is None:
        raise ValueError("Could not find 'IR SPECTRUM' in the output file.")
    ...
    for line in lines[start_idx + 1 :]:
        if PATTERNS["ir_end"].match(line):
            break

        m = PATTERNS["ir_row"].match(line)
        if not m:
            continue

        freqs.append(float(m.group("freq")))
        eps.append(float(m.group("eps")))
        intens.append(float(m.group("int")))
        t2.append(float(m.group("t2")))

    if not freqs:
        raise ValueError("Found 'IR SPECTRUM' but no data rows were parsed.")

    ir_values = {"frequencies": freqs, "eps": eps, "intensity": intens, "t2": t2}
    stats = {
        key: {
            "mean": np.mean(values),
            "max": np.max(values),
        ...
        for key, values in ir_values.items()
    }
    return ir_values, stats

def get_NMR_shieldings(out_file: str) -> dict:
    lines = Path(out_file).read_text(errors="ignore").splitlines()

    start_idx = None
    for i, line in enumerate(lines):
        if PATTERNS["nmr_start"].match(line):
            start_idx = i

    if start_idx is None:
        raise ValueError("Could not find 'CHEMICAL SHIELDING SUMMARY (ppm)' in the output file.")

    rows = []
    for line in lines[start_idx + 1 :]:
        if PATTERNS["nmr_end"].match(line):
            break

        m = PATTERNS["nmr_row"].match(line)
        if not m:
            continue
        ...
        aniso = float(m.group("aniso"))
        rows.append((idx, element, iso, aniso))

    if not rows:
        raise ValueError("Found NMR summary block, but no shielding rows were parsed.")

    max_idx = max(idx for idx, _, _, _ in rows)

    proton_isotropic = [None] * (max_idx + 1)
    proton_anisotropic = [None] * (max_idx + 1)
    carbon_isotropic = [None] * (max_idx + 1)
    carbon_anisotropic = [None] * (max_idx + 1)

    for idx, element, iso, aniso in rows:
        if element == "H":
            proton_isotropic[idx] = iso
            proton_anisotropic[idx] = aniso
        elif element == "C":
            carbon_isotropic[idx] = iso
            carbon_anisotropic[idx] = aniso

    return {
        "proton_isotropic": proton_isotropic,
        "proton_anisotropic": proton_anisotropic,
        "carbon_isotropic": carbon_isotropic,
        "carbon_anisotropic": carbon_anisotropic,
    }
    
def get_dipole_moment(output_file: str) -> dict:
    lines = Path(output_file).read_text(errors="ignore").splitlines()

    start_idx = None
    for i, line in enumerate(lines):
        if PATTERNS["dipole_start"].match(line):
            start_idx = i

    if start_idx is None:
        raise ValueError("Could not find 'DIPOLE MOMENT' in the output file.")

    dipole_x = dipole_y = dipole_z = None
    dipole_magnitude = None

    for line in lines[start_idx + 1 :]:
        if PATTERNS["dipole_end"].match(line):
            break

        total_m = PATTERNS["dipole_total_row"].match(line)
        if total_m:
            dipole_x = float(total_m.group("x"))
            dipole_y = float(total_m.group("y"))
            dipole_z = float(total_m.group("z"))
            continue

        mag_m = PATTERNS["dipole_mag"].match(line)
        if mag_m:
            dipole_magnitude = float(mag_m.group("mag"))

    if dipole_x is None or dipole_y is None or dipole_z is None:
        raise ValueError("Could not parse 'Total Dipole Moment' from the output file.")
    if dipole_magnitude is None:
        raise ValueError("Could not parse 'Magnitude (Debye)' from the output file.")
...
def get_isotropic_polarizability(output_file: str) -> dict:
    lines = Path(output_file).read_text(errors="ignore").splitlines()

    start_idx = None
    for i, line in enumerate(lines):
        if PATTERNS["polar_start"].match(line):
            start_idx = i

    if start_idx is None:
        raise ValueError(
            "Could not find 'STATIC POLARIZABILITY TENSOR (Dipole/Dipole)' in the output file."
        )

    isotropic_polarizability = None
    for line in lines[start_idx + 1 :]:
        m = PATTERNS["polar_value"].match(line)
        if m:
            isotropic_polarizability = float(m.group("value"))
            break

    if isotropic_polarizability is None:
        raise ValueError("Could not parse 'Isotropic polarizability' from the output file.")

    return {"isotropic_polarizability": isotropic_polarizability}

def get_Fukui_indices(cationic_nbo, anionic_nbo, neutral_nbo: str) -> dict:
    neutral_atoms = get_natural_population_analysis(neutral_nbo)
    cation_atoms = get_natural_population_analysis(cationic_nbo)
    anion_atoms = get_natural_population_analysis(anionic_nbo)

    q_neutral = [a["natural_charge"] for a in neutral_atoms]
    q_cation = [a["natural_charge"] for a in cation_atoms]
    q_anion = [a["natural_charge"] for a in anion_atoms]

    if not (len(q_neutral) == len(q_cation) == len(q_anion)):
        raise ValueError("Cationic, neutral, and anionic outputs do not have the same number of atoms.")

    f_plus = [q_neutral[i] - q_anion[i] for i in range(len(q_neutral))]
    f_minus = [q_cation[i] - q_neutral[i] for i in range(len(q_neutral))]
...
def _extract_final_single_point_energy(output_file: str) -> float:
    ...
    energy = None
    for line in lines:
        m = PATTERNS["final_sp_energy"].match(line)
        if m:
            energy = float(m.group("energy"))

    if energy is None:
        raise ValueError(f"Could not find 'FINAL SINGLE POINT ENERGY' in {output_file}")

    return energy


def get_IE_EA(cationic_output: str, anionic_output: str, neutral_output: str) -> dict:
    cation_energy = _extract_final_single_point_energy(cationic_output)
    anion_energy = _extract_final_single_point_energy(anionic_output)
    neutral_energy = _extract_final_single_point_energy(neutral_output)

    ionization_energy = (cation_energy - neutral_energy) * 27.211386
    electron_affinity = (neutral_energy - anion_energy) * 27.211386

    hardness = (ionization_energy - electron_affinity) / 2
    chemical_potential = -(ionization_energy + electron_affinity) / 2
...
def get_all_catalyst_data(smiles, xyz_file, neutral_output, cationic_output, anionic_output, neutral_nbo, cationic_nbo, anionic_nbo, nmr_output, IR_output, dipole_polarizability_output, homo_lumo_output):
    """
    Retrieves all featuresyrup data in one pass. 
    
    Args: 
        smiles (str): SMILES string of the molecule.
        xyz_file (str): Path to the geometry (.xyz) file.
        ...
        homo_lumo_output (str): Path to the HOMO-LUMO output file.
    Returns:
        dict: Dictionary containing all catalyst data.
    """
    data = {}
...
def _require_existing_paths(required_paths: dict[str, Union[str, Path]]) -> None:
    missing_paths = [name for name, path in required_paths.items() if path is None or not Path(path).exists()]
    if missing_paths:
        raise FileNotFoundError(
            f"Missing required catalyst inputs: {', '.join(missing_paths)}"
        )
...
def generate_qm_data_dict(
    ...
    neutral_output: Union[str, Path],
    cationic_output: Union[str, Path],
    anionic_output: Union[str, Path],
    ...
    dipole_polarizability_output: Union[str, Path, None] = None,
):
    """
    ...
    Args:
        ...
        dipole_polarizability_output (str | Path | None, optional): Path to the dipole and polarizability output file, if available.
    Returns:
    ...
    required_paths = {
        ...
        "cationic_output": cationic_output,
        "anionic_output": anionic_output,
    ...
    if dipole_polarizability_output is not None:
        required_paths["dipole_polarizability_output"] = dipole_polarizability_output
    ...
    dipole_polarizability_output = Path(dipole_polarizability_output) if dipole_polarizability_output else None
    neutral_nbo = Path(neutral_nbo)
    cationic_nbo = Path(cationic_nbo)
