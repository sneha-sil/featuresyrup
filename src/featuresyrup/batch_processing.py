...
from .functions import generate_qm_data_dict, clear_file_cache, clear_mmap_cache
...
class MoleculeData:
    ...
    def __init__(self, mol_id: str, labels: dict = None):
        ...
        self.cationic_output = None
        self.anionic_output = None
        self.neutral_nbo = None
        self.cationic_nbo = None
        self.anionic_nbo = None
        self.nmr_output = None
        self.IR_output = None
    ...
    def to_dict(self):
        ...
        return {
            ...
            'janpa': self.janpa,
            'nbo': self.nbo,
            'neutral_output': self.neutral_output,
            'cationic_output': self.cationic_output,
            'anionic_output': self.anionic_output,
            'neutral_nbo': self.neutral_nbo,
            'cationic_nbo': self.cationic_nbo,
            'anionic_nbo': self.anionic_nbo,
            'nmr_output': self.nmr_output,
...
class BatchProcessor:
    ...
    def run(self):
        ...
        logger.info("Starting featuresyrup batch processing with optimizations...")
    ...
    def _discover_files_in_directory(self, base_dir: Path) -> Dict[str, List[Path]]:
        ...
        pattern_map = {
            ...
            'janpa': 'janpa_pattern',
            'nbo': 'nbo_pattern',
            'neutral_output': 'neutral_output_pattern',
            'cationic_output': 'cationic_output_pattern',
            'anionic_output': 'anionic_output_pattern',
            'neutral_nbo': 'neutral_nbo_pattern',
            'cationic_nbo': 'cationic_nbo_pattern',
            'anionic_nbo': 'anionic_nbo_pattern',
    ...
    def _process_chunk_optimized(self, molecules_chunk: List[MoleculeData]):
        ...
        for mol_data in molecules_chunk:
            mol_dict = {
                ...
                'neutral_output': str(mol_data.neutral_output) if mol_data.neutral_output else None,
                'cationic_output': str(mol_data.cationic_output) if mol_data.cationic_output else None,
                'anionic_output': str(mol_data.anionic_output) if mol_data.anionic_output else None,
                'neutral_nbo': str(mol_data.neutral_nbo) if mol_data.neutral_nbo else None,
                'cationic_nbo': str(mol_data.cationic_nbo) if mol_data.cationic_nbo else None,
                'anionic_nbo': str(mol_data.anionic_nbo) if mol_data.anionic_nbo else None,
                'nmr_output': str(mol_data.nmr_output) if mol_data.nmr_output else None,
                'IR_output': str(mol_data.IR_output) if mol_data.IR_output else None,
                'dipole_polarizability_output': str(mol_data.dipole_polarizability_output) if mol_data.dipole_polarizability_output else None,
                'homo_lumo_output': str(mol_data.homo_lumo_output) if mol_data.homo_lumo_output else None,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='featuresyrup batch processing script',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python -m featuresyrup.batch_processing --config /path/to/config.yaml
